...
import type { LinkCategory } from '@flou/shared'
...
export function CategoryDialog({ category, presetColors, onSave, onDelete, onClose }: CategoryDialogProps) {
  const [name, setName] = useState(category?.name ?? '')
  const [color, setColor] = useState(category?.color ?? presetColors[0]!)
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (!trimmed) return
    setSaving(true)
    try { await onSave(trimmed, color, category?.id) } finally { setSaving(false) }
  }

  async function handleDelete() {
    if (!category) return
    if (!confirm(`Delete category "${category.name}"? Links inside will become uncategorized.`)) return
    setDeleting(true)
    try { await onDelete(category.id) } finally { setDeleting(false) }
  }
          ...
          <form onSubmit={handleSave}>
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <input
                  ...
                  maxLength={60} placeholder="Category name"
                  style={{ width: '100%', fontSize: 14, padding: '8px 12px', borderRadius: 8, border: '1.5px solid var(--divider)', background: 'var(--bg2)', color: 'var(--text)', outline: 'none', boxSizing: 'border-box' }}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {presetColors.map(c => (
                    <button key={c} type="button" onClick={() => setColor(c)}
                      style={{ width: 24, height: 24, borderRadius: '50%', background: c, cursor: 'pointer', border: `2.5px solid ${color === c ? 'var(--text)' : 'transparent'}`, outline: color === c ? '2px solid var(--bg)' : 'none', outlineOffset: -3 }}
                    />
                  ))}
                </div>
                {/* Preview */}
            ...
            <div className="flex-wrap sm:flex-nowrap" style={{ padding: '12px 20px', borderTop: '1px solid var(--divider)', display: 'flex', gap: 8 }}>
              ...
              <button type="submit" disabled={saving || !name.trim()}
                style={{ padding: '7px 18px', borderRadius: 8, fontSize: 12, fontWeight: 500, background: 'var(--accent)', color: '#fff', cursor: 'pointer', opacity: saving || !name.trim() ? 0.6 : 1 }}>
                {saving ? 'Saving…' : 'Save'}
              </button>
            </div>
          </form>
