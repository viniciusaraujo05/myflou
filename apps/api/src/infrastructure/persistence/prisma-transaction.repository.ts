...
export class PrismaTransactionRepository implements ITransactionRepository {
  ...
  async create(userId: string, data: { type: TransactionType; amount: number; category: string; description?: string | null; date: Date }) {
    const row = await this.prisma.transaction.create({ data: { userId, ...data } })
    return toEntity(row)
  }

  async findByUserId(userId: string, filters?: { from?: Date; to?: Date }) {
  ...
  async update(id: string, data: Partial<{ type: TransactionType; amount: number; category: string; description: string | null; date: Date }>) {
